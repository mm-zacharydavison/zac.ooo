 ...
 */
export async function loadRoot(me: Account | undefined | null): Promise<JazzId | undefined> {
	if (!me) return
	const group = await loadRootGroup(me, GLOBAL_GROUP_ID)

	// Find existing global container.
	const globalContainerId = await GlobalContainer.findUnique(GLOBAL_CONTAINER_ID, group.id)
